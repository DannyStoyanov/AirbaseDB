...
public class Utils {
    ...
    public static void areValidCreateCommandArguments(ArrayList<String> args) throws ArgumentsException {
        if (!isNumeric(args.get(0))) {
            throw new ArgumentsException("Airplane \"ID\" is not is not a number.");
        } else if (!isNumeric(args.get(3))) {
            throw new ArgumentsException("Airplane \"flights\"  is not a number.");
        }
        try {
            ...
            if (!isValidNumber(id)) {
                throw new ArgumentsException("Airplane \"ID\" is not is not a valid number.");
            } else if (!isValidString(name)) {
                throw new ArgumentsException("Airplane \"name\"  is not valid string.");
            } else if (!isValidString(type)) {
                throw new ArgumentsException("Airplane \"type\"  is not valid string.");
            } else if (!isValidNumber(flights)) {
                throw new ArgumentsException("Airplane \"flights\"  is not a valid number.");
            }
    ...
    public static void areValidShowCommandArguments(ArrayList<String> args) throws ArgumentsException {
        if (!isNumeric(args.get(0))) {
            throw new ArgumentsException("show command \"offset\" is not is not a number.");
        } else if (!isNumeric(args.get(1))) {
            throw new ArgumentsException("show command \"limit\"  is not a number.");
        } else if (!notValidOffsetAndLimit(Integer.parseInt(args.get(0)), Integer.parseInt(args.get(1)))) {
            throw new ArgumentsException("show command \"offset\" and \"limit\" are not valid.");
        }
    }
}
